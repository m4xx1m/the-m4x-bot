...
class DataBase:
    def __init__(self, lang_cfgs: dict = None, bot_config: dict = None):
        self.con = sqlite3.connect(bot_config['db_name'])
        self.stock_limit = bot_config['stock_limit']
    ...
    def run_updates(self, timeout: int = 5):
        if not self._upd_task or self._upd_task.cancelled():
            self._upd_task = asyncio.get_event_loop().create_task(self._run_updates(timeout))
            return True
        else:
            return False

    def stop_updates(self) -> bool:
        if self._upd_task or not self._upd_task.cancelled():
            self._upd_task.cancel()
            return True
        elif self._upd_task.cancelled():
            return True
        elif self._upd_task is None:
            return True
        else:
            return False
